...
function jsonSchemaToZod(schema) {
    ...
    if (t === 'object') {
        ...
        return base.superRefine((data, ctx) => {
            for (const r of req)
                if (data[r] === undefined)
                    ctx.addIssue({ code: z.ZodIssueCode.custom, message: `Missing required field: ${r}` });
        });
    }
    return z.any();
}
...
function pickParamsForPath(path) {
...
export function generateTools(catalog, dc, policy, options) {
    const rest = dc['rest'];
    const tools = [];
    for (const entry of catalog) {
        const pack = entry.pack ?? 'CORE';
        if (!options.packsEnabled.has(pack))
            continue;
        ...
        const handler = {
            ...
            async *handler({ input }) {
                ...
                if (data.channel_id && !policy.allowChannel(String(data.channel_id)))
                    throw new Error('Channel not allowed by policy');
                if (data.guild_id && !policy.allowGuild(String(data.guild_id)))
                ...
                const result = await rest[verb](route, restOptions);
